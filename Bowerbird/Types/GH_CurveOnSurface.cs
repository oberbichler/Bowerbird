...
public class GH_CurveOnSurface : GH_GeometricGoo<IOrientableCurve>, IGH_PreviewData, IGH_BakeAwareData
{
    ...
    public override string TypeDescription => "Curve on Surface geometry type";

    public BoundingBox ClippingBox { get; private set; }

    public override BoundingBox Boundingbox => ClippingBox;

    public override IGH_Goo Duplicate() => new GH_CurveOnSurface(Value);
    ...
    public bool BakeGeometry(RhinoDoc doc, ObjectAttributes att, out Guid objGuid)
    {
        ...
        if (Geometry != null)
        {
            objGuid = doc.Objects.AddCurve(Geometry, att);
    ...
    public override IGH_GeometricGoo DuplicateGeometry() => new GH_CurveOnSurface(Value);

    public override BoundingBox GetBoundingBox(Transform xform) => xform.TransformBoundingBox(ClippingBox);

    public override IGH_GeometricGoo Transform(Transform xform) => new GH_CurveOnSurface(Value.Transform(xform));

    public override IGH_GeometricGoo Morph(SpaceMorph xmorph) => new GH_CurveOnSurface(Value.Morph(xmorph));
}
