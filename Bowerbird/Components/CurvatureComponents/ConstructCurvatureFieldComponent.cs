...
{
    public ConstructCurvatureFieldComponent() 
        : base("BB Curvature Field", "Field", "Show the direction field for a given path type. The field can be displayed in the geometry or parameter space (see context menu).", "Bowerbird", "Paths")
    {
        UpdateMessage();
    }
            ...
            if (face.IsSurface)
            {
                ...
                foreach (var u in us)
                {
                    foreach (var v in vs)
                    {
                        ...
                        {
                            var x = new Point3d(domainU.ParameterAt(u), domainV.ParameterAt(v), 0.0);
            ...
            else
            {
                ...
                foreach (var mesh in meshs)
                {
                    foreach (var vertex in mesh.TopologyVertices)
                    {
                        ...
                        if (!path.Directions(surface, uv, out _, out _, out var d1, out var d2))
                            continue;
    ...
    protected override System.Drawing.Bitmap? Icon => Bowerbird.Properties.Resources.icon_field;

    public override GH_Exposure Exposure => GH_Exposure.primary;

    public override Guid ComponentGuid => new("{E0660D1B-F785-4242-813F-CAB730C93DE5}");
}
