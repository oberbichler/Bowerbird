...
public class TrimmedBoundary : IBoundary
{
    ...
    public bool Clip(Vector2d a, ref Vector2d b)
    {
        ...
        {
            // Create 2D-Segment and ensure proper disposal of unmanaged Rhino Curve
            using (var line = new LineCurve(new Line(a.X, a.Y, 0, b.X, b.Y, 0)))
            {
        ...
        Debug.Assert(closestPoint.Point.DistanceTo(location) < 1e-3);

        var adjacentUV = adjacentTrim.PointAt(closestPoint.T);
