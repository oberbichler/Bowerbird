...
public class Pathfinder
{
    ...
    private struct Task
    {
        public Task(BrepFace face, Vector2d parameter, Point3d location, Vector3d direction, bool flip)
        {
            Face = face;
            Parameter = parameter;
            Location = location;
    ...
    private static void AddTask(Queue<Task> tasks, BrepFace? adjacentFace, Vector2d adjacentUV, Point3d location, Vector3d direction, bool flip)
    {
        if (adjacentFace == null)
            return;

        tasks.Enqueue(new Task(adjacentFace, adjacentUV, location, direction, flip));
    }
    ...
    public static List<Pathfinder> Create(Path path, BrepFace face, Vector2d uv, bool type, double stepSize, double tolerance, int maxPoints, double loopTolerance)
    {
        ...
        if (maxPoints <= 0)
            throw new System.ArgumentException("Maximum points must be > 0", nameof(maxPoints)); // FIXED: copy-paste parameter mismatch bug in original code!
        ...
        // Initial face
        {
            var parameters = new List<Point3d> { new(uv.X, uv.Y, 0) };
            var points = new List<Point3d> { face.PointAt(uv.X, uv.Y) };

            var direction = path.InitialDirection(face, uv, type);
            if (direction.IsZero)
                return new List<Pathfinder>();

            IBoundary boundary = TrimmedBoundary.Create(face);
            ...
            // First branch
            {
                ...
                AddTask(tasks, adjacentFace, boundary.AdjacentUV, endLocation, endDirection, true);
            ...
            // Second branch
            {
                ...
                AddTask(tasks, adjacentFace, boundary.AdjacentUV, endLocation, endDirection, false);
                breakpoints.Add(endLocation);
            }
        ...
        while (tasks.Count != 0)
        {
            ...
            var parameters = new List<Point3d> { new(task.Parameter.X, task.Parameter.Y, 0) };
            var points = new List<Point3d> { task.Location };

            IBoundary boundary = TrimmedBoundary.Create(task.Face);
            ...
            AddTask(tasks, adjacentFace, boundary.AdjacentUV, endLocation, endDirection, task.Flip);
            breakpoints.Add(endLocation);
    ...
    private static Vector3d FindPath(List<Point3d> parameters, List<Point3d> points, Path path, Surface surface, Vector2d uv, Vector3d direction, IBoundary boundary, double stepSize, double tolerance, int maxPoints)
    {
        while (points.Count < maxPoints)
        {
            ...
            var delta = (d0 + 2.0 * d1 + 2.0 * d2 + d3) / 6.0;
