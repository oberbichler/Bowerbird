...
public class UntrimmedBoundary : IBoundary
{
    ...
    private readonly Vector3d _x0;
    private readonly Vector3d _x1;
    private readonly Vector3d _x2;
    private readonly Vector3d _x3;
    ...
    private void SetAdjacentFace(Vector2d uv)
    {
        ...
        if (boundingTrim == null || !boundingTrim.ClosestPoint(uvLocation, out var t))
        ...
        if (adjacentTrims.Length > 1)
        {
            ...
            else
            {
                ...
                if (!adjacentTrim.ClosestPoint(uvLocationAdjacent, out var tClose))
                    throw new Exception("Projection failed");

                AdjacentUV = new Vector2d(adjacentU, adjacentV);

                Debug.Assert(adjacentTrim.PointAt(tClose).DistanceTo(new Point3d(adjacentU, adjacentV, 0)) < 1e-3);
            }
        }
