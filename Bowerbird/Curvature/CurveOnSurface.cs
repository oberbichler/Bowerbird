...
public class CurveOnSurface : IOrientableCurve
{
    ...
    {
        var cT = Curve.TangentAt(t);
        var uv = Curve.PointAt(t);
        Surface.Evaluate(uv.X, uv.Y, 1, out var _, out var sUv);
        var xT = cT.X * sUv[0] + cT.Y * sUv[1];
        return xT / xT.Length;
    }
    ...
    public double TorsionAt(double t)
    {
        ...
        var x3 = s10 * u3 + 3 * s12 * u1 * Pow(v1, 3) + s03 * Pow(v1, 3) + 3 * v1 * (s21 * Pow(u1, 2) + s11 * u2 + s02 * v2) + u1 * (s30 * Pow(u1, 2) + 3 * s20 * u2 + 3 * s11 * v2) + s01 * v3;
    ...
    public IOrientableCurve Reparameterized()
    {
        ...
        return Create(Surface, curve) ?? this;
    }
    ...
    private List<(double T, Point3d Point)> Tessellation(double tolerance)
    {
        const int maxIter = 10;
        var length = 0.0;

        var tessellation = new List<(double T, Point3d Point)>();

        for (int i = 0; i < Curve.SpanCount; i++)
        {
            var span = Curve.SpanDomain(i);
            var samplePoints = new List<(double T, Point3d Point)>();
            ...
            samplePoints.Sort((a, b) => -a.T.CompareTo(b.T));
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    var maxDistance = 0.0;
                    var maxPoint = default((double T, Point3d Point));

                    var domain = new Interval(a.T, b.T);
                    var t = domain.ParameterAt(0.5);
                    var point = PointAt(t);

                    var distance = new Line(a.Point, b.Point).DistanceTo(point, true);
    ...
    public (double T, Point3d Point) Invert(Point3d sample, double tolerance)
    {
        var tessellation = Tessellation(tolerance);
        var polyline = new Polyline(tessellation.Select(o => o.Point));
        ...
        var minParameter = tessellation[Max(0, closestSpan - 1)].T;
        var maxParameter = tessellation[Min(tessellation.Count - 1, closestSpan + 2)].T;
        ...
        var t0 = tessellation[closestSpan].T;
        var t1 = tessellation[closestSpan + 1].T;
